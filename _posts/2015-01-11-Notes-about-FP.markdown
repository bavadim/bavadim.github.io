---
layout: post
title: "Заметки о функциональном программировании. Часть 1"
comments: true
categories: programming
---

Всем привет. Последнее время функциональное программирование (далее ФП) стремительно входит в жизнь рядового разработчика. Последние версии таких императивных языков как C++ и Java с недавних пор включают механизмы работы с анонимными функциями. Умные дяди говорят, что теперь на Java можно писать в функциональном стиле. К сожалению практика показывает, что понимание функционального стиля часто сводится к написанию императивного кода, но с анонимными процедурами. В итоге, с таким "функциональным стилем", есть риск резко опуститься на дно говнокода.
...
![My helpful screenshot]({{ site.url }}/assets/lambda.svg)
...
Есть еще всем известное объектно-ориентированное программирование (ООП). Но на мой взгляд парадигмой оно не является, так как по меньшей мере не определяет модель вычислений, хотя включает в себя идеи организации данных и диктует определенный способ мышления (я считаю, что иногда далеко не самый лучший). Основные идеи ООП не противоречат идеям перечисленных выше парадигм программирования. Поэтому идеями ООП можно безболезненно дополнить любую из приведенных выше парадигм. Например функциональное программирование (как это сделать рассмотрим позже, в следующих статьях).
...
- управляющие структуры типа `if-then-else`, `return`, цыклы
...
В функциональных программах присутствуют аналоги условных выражений, в этом они похожи на императивные.

Таким образом функциональные и императивные программы отличаются в следующем:
...
Очевидно, что при вызове по имени x будет рассчитываться 3 раза:

	f(const) -> const + const + const -> 1 + const + const -> 1 + 1 + const -> 1 + 1 + 1 -> 3
...
Картина вычислений немного отличаются. Если в примере с алгоритмом Евклида на каждом шаге мы делали однотипные вычисления, то в примере с факториалом на каждом шаге формула меняется. Если точнее, то в первом примере последний рекурсивный вызов - просто вызов функцией самой себя, в то время как во втором примере - вызов функцией самой себя и какие-то действия (умножение). Первый случай имеет специальное название - это хвостовая рекурсия.
...
Мы просто ввели аргумент-аккумулятор вместо сохранения результата в стековом кадре. Компилятор выполнит оптимизацию автоматически. Полученный байт-код будет аналогичен коду, полученному из итерации на императивном языке, например на Java. 

## Что дальше

На этом можно было бы все закончить, так как фундаментальные концепции ФП мы рассмотрели. Но как известно демон кроется в деталях. В следующих статьях мы рссмотрим функции высшего порядка, ООП, напишем коллекции в функциональном стиле и много другое. Спасибо за внимание, до новых встреч.
